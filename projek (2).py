...
def kapal_selam() :
    ...
    glPushMatrix()
    # print(pos_x_kapalselam,pos_y_hiu)
    #memberi pembatas
    if not selesai:
        if pos_y_kapalselam > batas_atas-80 :
            pos_y_kapalselam = batas_atas-80
        if pos_y_kapalselam < -batas_atas-90:
            pos_y_kapalselam = -batas_atas-90
        if pos_x_kapalselam > batas_kiri+243 :
            pos_x_kapalselam = batas_kiri+243
        if pos_x_kapalselam < -batas_kiri+131:
            pos_x_kapalselam = -batas_kiri+131
...
def bottons(key,x,y) :
    global pos_y_kapalselam, pos_x_hiu, pos_y_hiu, pos_x_peluru, tembak, kecepatan_peluru, pos_x_kapalselam
    if key == GLUT_KEY_UP:
        pos_y_kapalselam += 7
    ...
    elif key == b'd': pos_x_kapalselam += 7
    elif key == b'a': pos_x_kapalselam -= 7
